...
import { IPropDef, IDefinitionClasses, IDefinitionEnums } from './baseInterfaces';
...
export function requestByTemplate(
  name: string,
  requestSchema: IRequestSchema,
  { enums, models }: Definitions
) {
  let {
    summary = '',
    parameters = '',
 ...
 * ${summary || ''}
 */
export const ${camelcase(name)} = async (${parameters}options:IRequestOptions={}) => {
  try {
    const configs:IRequestConfig = {...options, method: "${method}" };
    configs.headers = {
      ...options.headers,
...
export function serviceByTemplate(name: string, body: string) {
  return `
  /******* Generated by flipdishbytes/swagger-axios-codegen *******/
